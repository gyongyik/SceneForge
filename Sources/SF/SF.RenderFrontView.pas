...
type
  ...
  TRenderFrontView = class(TRenderFlatView)
...
procedure TRenderFrontView.FormActivate(Sender: TObject);
begin
  ActiveViewMode := vmFront;
end;

procedure TRenderFrontView.FormCreate(Sender: TObject);
begin
  inherited;
  Cursor := crCross;
end;

procedure TRenderFrontView.FormDeactivate(Sender: TObject);
begin
  FIsActive := False;
  Caption := 'Front View';
end;

procedure TRenderFrontView.MouseMoveCreation(Shift: TShiftState; X, Y: Integer);
var
  NP, D: TVertex;
  Obj: TObject3D;
begin
  case DefaultObject of
    otPolygon:
      ...
      begin
        D := TVertex.Create(Base.X + (P.X * PX), Base.Y + SY - (P.Y * PY), 0);
    ...
    otWedge:
      begin
        ...
        NP := TVertex.Create(Round(Base.X + (X * PX)), Round(Base.Y + SY - (Y * PY)), 0);
        D := TVertex.Create(NP.X - Obj.Center.X, Obj.Center.Y - NP.Y, 0);
    ...
    otCylinder:
      begin
        Obj := Scene.Objects.Last;
        NP := TVertex.Create(Round(Base.X + (X * PX)), Round(Base.Y + SY - (Y * PY)), 0);
        D := TVertex.Create(Obj.Center.X - NP.X, Obj.Center.Y - NP.Y, 0);
        TCylinder(Obj).ScaleByRadius(D, True, True, False);
        D.Free;
        NP.Free;
      end;
    otDisc:
      begin
        ...
        NP := TVertex.Create(Round(Base.X + (X * PX)), Round(Base.Y + SY - (Y * PY)), 0);
        D := TVertex.Create(Obj.Center.X - NP.X, Obj.Center.Y - NP.Y, 0);
        TDisc(Obj).ScaleByRadius(D, True, True, False);
        D.Free;
        NP.Free;
      end;
    otSphere:
...
procedure TRenderFrontView.MouseUpCreation(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
  P1, P2, D: TVertex;
  TX, TY, GS: Integer;
  Box: TBox;
  ...
  case DefaultObject of
    otRectangle:
      ...
      begin
        ...
        P1 := TVertex.Create(Round(Base.X + (X1 * PX)), Round(Base.Y + SY - (Y1 * PY)), 0);
        P2 := TVertex.Create(Round(Base.X + (P.X * PX)), Round(Base.Y + SY - (P.Y * PY)), Defaultdepth);
        D := TVertex.Create(Abs(P2.X - P1.X), Abs(P2.Y - P1.Y), Abs(P2.Z - P1.Z));
        if (D.X > 2) and (D.Y > 2) then
        begin
          Grid := TRectangle.CreateRectangle(Integer(DefaultSubX), Integer(DefaultSubY), P1, P2, vmFront, Scene.TextureManager.SelectedTexture, DefaultRectDoubleSided);
          Grid.AutoUV := True;
    ...
    otBox:
      begin
        ...
        P1 := TVertex.Create(Round(Base.X + (X1 * PX)), Round(Base.Y + SY - (Y1 * PY)), 0);
        P2 := TVertex.Create(Round(Base.X + (P.X * PX)), Round(Base.Y + SY - (P.Y * PY)), 128);
        D := TVertex.Create(Abs(P2.X - P1.X), Abs(P2.Y - P1.Y), Abs(P2.Z - P1.Z)); // delta
        if (D.X > 2) and (D.Y > 2) then
        begin
          Box := TBox.CreateBox(vmFront, P1, P2, Integer(DefaultDivX), Integer(DefaultDivY), Integer(DefaultDivZ), Scene.TextureManager.SelectedTexture);
          Box.AutoUV := True;
...
procedure TRenderFrontView.MouseUpSelection(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
  P1, P2, C: TVertex;
  TX, TY: Integer;
begin
  ...
  begin
    C := TVertex.Create(Base.X + (X * PX), Base.Y + SY - (Y * PY), 0);
    if ssAlt in Shift then // alt + left click = select object
...
procedure TRenderFrontView.MouseMoveAction(Shift: TShiftState; X, Y: Integer);
var
...
begin
  case Scene.EditDimension of
    ...
    begin
      NP := TVertex.Create(Base.X + (X * PX), Base.Y + SY - (Y * PY), 0);
      ...
      case Scene.Operation of
        ...
        opRotate:
          ...
          begin
            ...
            TVertexRotate.Execute(VertexList, TVertex.Create(0, 0, P.X - X));
...
procedure TRenderFrontView.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
var
  D: TVertex;
begin
...
procedure TRenderFrontView.UpdateProjection(const Width, Height: Integer);
begin
  ...
  begin
    ...
    glOrtho(Base.X, Base.X + SX, Base.Y, Base.Y + SY, -1, 1);
  end;
end;

procedure TRenderFrontView.FormMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
  C: TVertex;
  I: Integer;
begin
  ...
  begin // left button
    ...
    if ((ssCtrl in Shift) or (ssAlt in Shift)) and (not Assigned(FPolygon)) then
      FEditStatus := esSelection // selection
    else if (Scene.SelectedObjects.Count = 0) then
    begin
      FEditStatus := esCreation; // creation
      FPolygon := MouseDownCreation(Button, Shift, C, FPolygon, vmFront);
    end
    else
    begin // action (has selection)
...
procedure TRenderFrontView.FormMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
var
  D: TVertex;
begin
  ...
  if FEditStatus = esNavigation then
    MouseMoveNavigation(Shift, SP, MP, vmFront)
  ...
  Caption := 'Front View (X, Y = ' + FloatToStr(Round(Base.X + (X * PX))) + ', ' + FloatToStr(Round(Base.Y + SY - (Y * PY))) + ')';
  P.X := X;
  P.Y := Y;
end;

procedure TRenderFrontView.FormMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  FShiftState := Shift;
  Cursor := crCross;
...
function TRenderFrontView.ConvertPoint(const X, Y: Integer): TInteger2;
begin
  Result.X := Trunc(Base.X + X * PX);
  Result.Y := Trunc(Base.Y + SY - Y * PY);
end;

procedure TRenderFrontView.DrawGrid(GridSize, SkipSize: Integer; const Color: TRGBColor);
var
...
procedure TRenderFrontView.DrawViewport(const ShouldMakeCurrent: Boolean);
var
  I, J, K: Integer;
  Obj: TObject3D;
  Face: TFace;
...
begin
  ...
  try
    ...
    P1 := TVertex.Create((Base.X + (SX1 * PX)), (Base.Y + SY - (SY1 * PY)), 0);
    P2 := TVertex.Create((Base.X + (SX2 * PX)), (Base.Y + SY - (SY2 * PY)), 0);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            V3 := Obj.AABB.VertexArray[7];
            V4 := Obj.AABB.VertexArray[4];
          ...
          begin
            ...
            begin
              ...
              begin
                Vert := Obj.Vertices.GetVertex(J);
                if (P1.Z = 0) and (P2.Z = 0) and (Vert.X > P1.X) and (Vert.X < P2.X) and (Vert.Y < P1.Y) and (Vert.Y > P2.Y) then
                begin
                  glBegin(GL_POINTS);
                  glVertex2f(Vert.X, Vert.Y);
                  glEnd;
            ...
            begin
              ...
              begin
                ...
                begin
                  glColor3f(ColorHighlightedLine.R, ColorHighlightedLine.G, ColorHighlightedLine.B);
                  glBegin(GL_LINES);
                  glVertex2f(Edge.StartVertex.X, Edge.StartVertex.Y);
                  glVertex2f(Edge.EndVertex.X, Edge.EndVertex.Y);
                  glEnd;
                  glColor3f(ColorHighlightedGrip.R, ColorHighlightedGrip.G, ColorHighlightedGrip.B);
                  glBegin(GL_POINTS);
                  glVertex2f(Vert.X, Vert.Y);
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  glColor3f(ColorHighlightedGrip.R, ColorHighlightedGrip.G, ColorHighlightedGrip.B);
                  glBegin(GL_POINTS);
                  glVertex2f(Face.Center.X, Face.Center.Y);
                  glEnd;
                end;
              end;
            end;
...
constructor TRenderFrontView.Create(Scene: TScene);
begin
  inherited Create;
  FIsActive := False;
  Self.Scene := Scene;
...
procedure TRenderFrontView.FormPaint(Sender: TObject);
begin
  DrawViewport;
end;

procedure TRenderFrontView.FormMouseWheel(Sender: TObject; Shift: TShiftState; WheelDelta: Integer; MousePos: TPoint; var Handled: Boolean);
begin
  FShiftState := Shift;
  inherited;
end;
